...
function SprintWorkspace({ data, today, defaultProject, onOpen, onSave, onDelete }: {
  ...
  return <section className="sprint-workspace">
    ...
    <div className="sprint-days">{days.map((date) => { const tasks = weekTasks.filter((task) => taskOccursOnDate(task, date)).sort(compareProjectTasks); const planned = tasks.reduce((sum, task) => sum + taskMinutesOnDate(task, date), 0); const remaining = 360 - planned; const isToday = date === today; return <section className={`sprint-day ${isToday ? "today" : ""} ${remaining < 0 ? "over-capacity" : ""}`} key={date}><header><div><small>{shortDay(date)}</small><strong>{new Date(`${date}T12:00:00Z`).getUTCDate()}</strong></div><span className="day-capacity"><b>{formatMinutes(planned)}</b> / 6 h<br/><em>{remaining >= 0 ? `${formatMinutes(remaining)} libres` : `${formatMinutes(Math.abs(remaining))} de más`}</em></span></header><div className="sprint-day-tasks">{tasks.map((task) => <div className={task.scheduledDate !== date ? "task-continuation" : ""} key={task.id}>{taskCard(task, true)}{task.scheduledDate !== date && <small>Continúa</small>}</div>)}</div><button className="sprint-add" onClick={() => openTask(undefined, date)}><Plus size={13} /> Añadir</button></section>; })}</div>
