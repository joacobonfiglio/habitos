...
function SprintWorkspace({ data, today, defaultProject, onOpen, onSave, onDelete }: {
  ...
  return <section className="sprint-workspace">
    ...
    <div className="sprint-calendar"><div className="sprint-days">{days.map((date) => { const planned = weekTasks.reduce((sum, task) => sum + taskMinutesOnDate(task, date), 0); const remaining = 360 - planned; const isToday = date === today; const dayAgenda = agendaItems.filter((task) => taskOccursOnDate(task, date)); return <section className={`sprint-day ${isToday ? "today" : ""} ${remaining < 0 ? "over-capacity" : ""}`} key={date}><header><div><small>{shortDay(date)}</small><strong>{new Date(`${date}T12:00:00Z`).getUTCDate()}</strong></div><span className="day-capacity"><b>{formatMinutes(planned)}</b> / 6 h<br/><em>{remaining >= 0 ? `${formatMinutes(remaining)} libres` : `${formatMinutes(Math.abs(remaining))} de más`}</em></span></header><button className="sprint-add" onClick={() => openTask(undefined, date)}><Plus size={13} /> Añadir</button>{dayAgenda.map((task) => <button className={`sprint-day-agenda ${task.itemType}`} key={task.id} onClick={() => openTask(task)}><b>{task.scheduledTime || "—"}</b><span>{task.title}</span></button>)}</section>; })}</div><div className="sprint-timeline" style={{ gridTemplateRows: `repeat(${Math.max(1, taskLanes.length ? Math.max(...taskLanes.map((item) => item.lane)) + 1 : 1)}, minmax(54px, auto))` }}>{taskLanes.map(({ task, start, end, lane }) => <div className="sprint-task-span" style={{ gridColumn: `${start} / ${end + 1}`, gridRow: lane + 1 }} key={task.id}>{taskCard(task, true)}</div>)}</div></div>
