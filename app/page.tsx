...
function SprintWorkspace({ data, today, defaultProject, onOpen, onSave, onDelete }: {
  ...
  const allWeekTasks = data.projectTasks.filter((task) => task.scheduledDate != null && task.scheduledDate >= weekStart && task.scheduledDate <= weekEnd);
  ...
  return <section className="sprint-workspace">
    ...
    <details className="sprint-history"><summary><CheckCircle2 size={15} /> Completadas esta semana ({completedTasks.length})</summary><div>{completedTasks.sort(compareProjectTasks).map(taskCard)}</div>{!completedTasks.length && <p>Aún no hay tareas completadas en este sprint.</p>}</details>
  </section>;
}
...
function ProjectTaskForm({ projects, project, record, defaultDate, defaultSprintWeek, busy, onSubmit }: { projects: Project[]; project?: Project; record?: ProjectTask; defaultDate?: string; defaultSprintWeek?: string; busy: boolean; onSubmit: (payload: Record<string, unknown>) => void }) {
  ...
  return <form className="record-form" onSubmit={(event) => { event.preventDefault(); onSubmit({ ...form, sprintWeek: startOfWeek(form.scheduledDate) }); }}><div className="program-context"><CalendarDays size={17} /><div><strong>Planificar una acción</strong><small>Una tarea siempre vive en un día y en su sprint</small></div></div><label>Proyecto (opcional)<select value={form.projectId} onChange={(event) => setForm({ ...form, projectId: event.target.value })}><option value="">Sin proyecto</option>{projects.map((item) => <option key={item.id} value={item.id}>{item.title}</option>)}</select></label><label>Tarea<input required value={form.title} onChange={(event) => setForm({ ...form, title: event.target.value })} placeholder="Define una acción concreta" /></label><label>Detalle<textarea value={form.description} onChange={(event) => setForm({ ...form, description: event.target.value })} placeholder="Información necesaria para completarla…" /></label><div className="form-grid"><label>Estado<select value={form.status} onChange={(event) => setForm({ ...form, status: event.target.value as ProjectTask["status"] })}><option value="todo">Por hacer</option><option value="doing">En curso</option><option value="done">Hecha</option></select></label><label>Prioridad<select value={form.priority} onChange={(event) => setForm({ ...form, priority: event.target.value })}><option value="low">Baja</option><option value="medium">Media</option><option value="high">Alta</option></select></label><label>Energía necesaria<select value={form.energy} onChange={(event) => setForm({ ...form, energy: event.target.value as ProjectTask["energy"] })}><option value="low">Baja · tarea ligera</option><option value="medium">Media · concentración normal</option><option value="high">Alta · trabajo profundo</option></select></label><label>Tiempo estimado (min)<input required type="number" min="5" max="1440" step="5" value={form.estimatedMinutes} onChange={(event) => setForm({ ...form, estimatedMinutes: event.target.value })} placeholder="Ej. 45" /></label><label>Día planificado<input required type="date" value={form.scheduledDate} onChange={(event) => schedule(event.target.value)} /><small className="field-help">El día define automáticamente el sprint semanal.</small></label><label>Fecha límite<input type="date" value={form.dueDate} onChange={(event) => setForm({ ...form, dueDate: event.target.value })} /></label></div><SubmitButton busy={busy} label={record ? "Guardar cambios" : "Crear tarea"} /></form>;
}
