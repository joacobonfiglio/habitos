...
function ProjectsView({ data, today, onOpen, onSave, onDelete }: {
  ...
  const dayTasks = data.projectTasks.filter((task) => task.status !== "done" && task.scheduledDate === today).sort((a, b) => (a.scheduledTime || "99:99").localeCompare(b.scheduledTime || "99:99") || compareProjectTasks(a, b));
  const unscheduled = data.projectTasks.filter((task) => task.status !== "done" && !task.scheduledDate).sort(compareProjectTasks);
  ...
  const weekTasks = data.projectTasks.filter((task) => task.status !== "done" && task.scheduledDate && task.scheduledDate >= weekStart && task.scheduledDate <= weekEnd);
  ...
  const taskButton = (task: ProjectTask) => <button className="planner-task-row" key={task.id} onClick={() => onOpen({ kind:"projectTask", projects:data.projects, record:task })}><span className={`priority-dot ${task.priority || "medium"}`} /><div><strong>{task.scheduledTime ? `${task.scheduledTime} · ` : ""}{task.title}</strong><small>{data.projects.find((project) => project.id === task.projectId)?.title ?? "Sin proyecto"} · {task.estimatedMinutes != null ? formatMinutes(task.estimatedMinutes) : "Sin estimación"}{task.goalId ? ` · ${data.planGoals.find((goal) => goal.id === task.goalId)?.title ?? "Objetivo"}` : ""}</small></div><span className={`energy-tag ${task.energy || "medium"}`}><Zap size={10}/>{energyLabel(task.energy)}</span></button>;
  return <div className="page-content subpage planner-product"><section className="planner-product-hero"><div><span className="section-label dark"><LayoutDashboard size={14}/> PLANNER</span><h2>Tu sistema para decidir<br/>y ejecutar con claridad.</h2><p>Una misma tarea se ve en tu día, tu semana, tu mes y el objetivo que ayuda a construir.</p></div><button className="primary-button" onClick={() => onOpen({ kind:"projectTask", projects:data.projects, defaultDate:today, defaultSprintWeek:weekStart })}><Plus size={16}/> Añadir tarea</button></section><div className="planner-nav" role="tablist">{nav.map(({id,label,icon:Icon}) => <button key={id} className={view === id ? "active" : ""} onClick={() => setView(id)}><Icon size={15}/>{label}{id === "inbox" && unscheduled.length > 0 && <b>{unscheduled.length}</b>}</button>)}</div>
  {view === "day" && <section className="planner-day"><div className="planner-day-heading"><div><span className="section-label dark">HOY · {formatLongDate(today)}</span><h3>Tu foco tiene espacio.</h3></div><div className="planner-load"><strong>{formatMinutes(dayTasks.reduce((sum,item) => sum + (item.estimatedMinutes ?? 0),0))}</strong><small>planificados de 6 h</small></div></div><div className="planner-day-grid"><section className="card planner-unscheduled"><div className="card-heading"><div><span className="section-label dark"><ListTodo size={14}/> SIN HORA FIJA</span><h3>Para hacer hoy</h3></div><button className="add-inline top" onClick={() => onOpen({kind:"projectTask",projects:data.projects,defaultDate:today,defaultSprintWeek:weekStart})}>+ Tarea</button></div>{dayTasks.map(taskButton)}{!dayTasks.length && <EmptyState text="No tienes tareas para hoy. Elige una del Inbox o crea una nueva." action="Planificar tarea" onClick={() => onOpen({kind:"projectTask",projects:data.projects,defaultDate:today,defaultSprintWeek:weekStart})}/>}</section><section className="card planner-timeline"><div className="card-heading"><div><span className="section-label dark"><Timer size={14}/> BLOQUES DE ENERGÍA</span><h3>Orden sugerido</h3></div></div><div className="energy-block high"><span>MAÑANA</span><strong>Alta energía</strong><small>Trabajo profundo y decisiones</small></div><div className="energy-block medium"><span>MEDIODÍA</span><strong>Energía media</strong><small>Gestión, llamadas y coordinación</small></div><div className="energy-block low"><span>TARDE</span><strong>Energía baja</strong><small>Cierres, tareas ligeras y revisión</small></div></section></div><section className="planner-checkout"><div><span>CIERRE DEL DÍA</span><strong>Al terminar, revisa lo pendiente y decide: mañana, Inbox o eliminar.</strong></div><button onClick={() => setView("inbox")}>Abrir pendientes →</button></section></section>}
...
function SprintWorkspace({ data, today, defaultProject, onOpen, onSave, onDelete }: {
  ...
  const allWeekTasks = data.projectTasks.filter((task) => task.scheduledDate != null && task.scheduledDate <= weekEnd && taskEndDate(task) >= weekStart);
  ...
  function taskCard(task: ProjectTask, compact = false) {
    const project = data.projects.find((item) => item.id === task.projectId);
    return <article className={`sprint-task-card ${task.status} ${task.priority === "high" ? "high-priority" : ""} ${compact ? "compact" : ""}`} key={task.id}>
      ...
      <button className="sprint-task-body" onClick={() => openTask(task)}><span>{project?.title ?? "Sin proyecto"}</span><strong>{task.title}</strong><small>{task.estimatedMinutes != null ? formatMinutes(task.estimatedMinutes) : "Sin estimación"} · <Zap size={10} /> {energyLabel(task.energy)}</small></button>
  ...
  return <section className="sprint-workspace">
    ...
    <div className="sprint-summary"><article><small>PENDIENTES DEL SPRINT</small><strong>{weekTasks.length}</strong><span>{completed} completadas en el registro</span></article><article><small>TIEMPO PENDIENTE</small><strong>{formatMinutes(estimated)}</strong><span>{weekTasks.filter((task) => task.estimatedMinutes == null).length} sin estimar</span></article><article><small>CAPACIDAD SEMANAL</small><strong>{formatMinutes(7 * 360 - estimated)}</strong><span>de 42 h efectivas disponibles</span></article><article><small>AVANCE</small><strong>{allWeekTasks.length ? `${Math.round(completed / allWeekTasks.length * 100)}%` : missingValue}</strong><span>{weekTasks.length ? `${weekTasks.length} por hacer` : "Sprint al día"}</span></article></div>
...
function RecordModal({ modal, data, today, close, save, navigate, open }: {
  ...
  return <div className="modal-backdrop" onMouseDown={close}><div className="quick-modal record-modal" onMouseDown={(event) => event.stopPropagation()}>
    ...
    {modal.kind === "projectTask" && <ProjectTaskForm projects={modal.projects} goals={data.planGoals} project={modal.project} record={modal.record} defaultDate={modal.defaultDate} defaultSprintWeek={modal.defaultSprintWeek} busy={busy} onSubmit={(payload) => submit("projectTask", payload, modal.record ? "Tarea actualizada" : "Tarea creada")} />}
...
function ProjectTaskForm({ projects, goals, project, record, defaultDate, defaultSprintWeek, busy, onSubmit }: { projects: Project[]; goals: PlanGoal[]; project?: Project; record?: ProjectTask; defaultDate?: string; defaultSprintWeek?: string; busy: boolean; onSubmit: (payload: Record<string, unknown>) => void }) {
  ...
  return <form className="record-form" onSubmit={(event) => { event.preventDefault(); onSubmit({ ...form, goalId: form.goalId || null, scheduledTime: form.scheduledTime || null, endDate: form.endDate || form.scheduledDate, sprintWeek: form.scheduledDate ? startOfWeek(form.scheduledDate) : null }); }}><div className="program-context"><CalendarDays size={17} /><div><strong>Planificar una acción</strong><small>Una tarea se conecta a un área, puede impulsar un objetivo y aparece en todas las vistas.</small></div></div><label>Área / proyecto<select required value={form.projectId} onChange={(event) => setForm({ ...form, projectId: event.target.value })}><option value="" disabled>Selecciona un área o proyecto</option>{projects.map((item) => <option key={item.id} value={item.id}>{item.area} · {item.title}</option>)}</select><small className="field-help">Toda tarea necesita contexto. Crea un proyecto si todavía no existe.</small></label><label>Objetivo que impulsa (opcional)<select value={form.goalId} onChange={(event) => selectGoal(event.target.value)}><option value="">Sin objetivo vinculado</option>{goals.filter((goal) => goal.status !== "done").map((goal) => <option key={goal.id} value={goal.id}>{goal.title}</option>)}</select></label><label>Tarea<input required value={form.title} onChange={(event) => setForm({ ...form, title: event.target.value })} placeholder="Define una acción concreta" /></label><label>Detalle<textarea value={form.description} onChange={(event) => setForm({ ...form, description: event.target.value })} placeholder="Información necesaria para completarla…" /></label><div className="form-grid"><label>Estado<select value={form.status} onChange={(event) => setForm({ ...form, status: event.target.value as ProjectTask["status"] })}><option value="todo">Por hacer</option><option value="doing">En curso</option><option value="done">Hecha</option></select></label><label>Prioridad<select value={form.priority} onChange={(event) => setForm({ ...form, priority: event.target.value })}><option value="low">Baja</option><option value="medium">Media</option><option value="high">Alta · destacar en agenda</option></select></label><label>Energía necesaria<select value={form.energy} onChange={(event) => setForm({ ...form, energy: event.target.value as ProjectTask["energy"] })}><option value="low">⚡ Baja · tarea ligera</option><option value="medium">⚡⚡ Media · concentración normal</option><option value="high">⚡⚡⚡ Alta · trabajo profundo</option></select></label><label>Tiempo estimado total (min)<input required type="number" min="5" max="1440" step="5" value={form.estimatedMinutes} onChange={(event) => setForm({ ...form, estimatedMinutes: event.target.value })} placeholder="Ej. 45" /></label><label>Empieza<input type="date" value={form.scheduledDate} onChange={(event) => schedule(event.target.value)} /><small className="field-help">Déjala vacía para mantenerla en Inbox.</small></label><label>Hora fija (opcional)<input type="time" value={form.scheduledTime} onChange={(event) => setForm({ ...form, scheduledTime: event.target.value })} /></label><label>Termina<input type="date" min={form.scheduledDate} value={form.endDate} onChange={(event) => setForm({ ...form, endDate: event.target.value })} /><small className="field-help">Mismo día si es una tarea puntual.</small></label><label>Fecha límite<input type="date" value={form.dueDate} onChange={(event) => setForm({ ...form, dueDate: event.target.value })} /></label></div><SubmitButton busy={busy} label={record ? "Guardar cambios" : "Crear tarea"} /></form>;
}
...
function saveLocalRecord(current: LifeData, resource: Resource, payload: Record<string, unknown>): LifeData {
  ...
  if (resource === "projectTask") {
    ...
    const status = payload.status === "done" ? "done" : payload.status === "doing" ? "doing" : "todo";
    const record = normalizeProjectTask({ ...payload, id, status, scheduledDate, endDate: payload.endDate || scheduledDate, sprintWeek: scheduledDate ? startOfWeek(scheduledDate) : payload.sprintWeek || null, dueDate: payload.dueDate || null, goalId: payload.goalId || null, scheduledTime: payload.scheduledTime || null, completedAt: status === "done" ? payload.completedAt || argentinaDateKey(new Date()) : null, estimatedMinutes: nullableNumber(payload.estimatedMinutes) } as unknown as ProjectTask);
...
function goalCompletion(goal: PlanGoal, tasks: PlanTask[], projectTasks: ProjectTask[] = []) {
  const linked = [...tasks.filter((task) => task.goalId === goal.id), ...projectTasks.filter((task) => task.goalId === goal.id)];
