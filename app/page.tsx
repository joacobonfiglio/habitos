...
function PlanningView({ data, today, onOpen, onSave, onDelete, embedded = false }: {
  ...
  return <div className={embedded ? "planning-page integrated-planning" : "page-content subpage planning-page"}>
    ...
    <div className="planning-summary">
      ...
      <article><span className="planning-summary-icon mint"><CheckCircle2 size={19} /></span><div><small>Tareas</small><strong>{completedTasks}/{visibleTasks.length}</strong><span>{totalProgress}% de avance</span></div></article>
...
function ProjectsView({ data, today, onOpen, onSave, onDelete }: {
  ...
  const dayTasks = data.projectTasks.filter((task) => task.status !== "done" && task.scheduledDate === today).sort(compareProjectTasks);
  ...
  const taskButton = (task: ProjectTask) => <button className="planner-task-row" key={task.id} onClick={() => onOpen({ kind:"projectTask", projects:data.projects, record:task })}><span className={`priority-dot ${task.priority || "medium"}`} /><div><strong>{task.title}</strong><small>{data.projects.find((project) => project.id === task.projectId)?.title ?? "Sin proyecto"} · {task.estimatedMinutes != null ? formatMinutes(task.estimatedMinutes) : "Sin estimación"}</small></div><span className={`energy-tag ${task.energy || "medium"}`}><Zap size={10}/>{energyLabel(task.energy)}</span></button>;
...
function saveLocalRecord(current: LifeData, resource: Resource, payload: Record<string, unknown>): LifeData {
  ...
  if (resource === "projectTask") {
    ...
    const record = normalizeProjectTask({ ...payload, id, scheduledDate, endDate: payload.endDate || scheduledDate, sprintWeek: scheduledDate ? startOfWeek(scheduledDate) : payload.sprintWeek || null, dueDate: payload.dueDate || null, estimatedMinutes: nullableNumber(payload.estimatedMinutes) } as unknown as ProjectTask);
